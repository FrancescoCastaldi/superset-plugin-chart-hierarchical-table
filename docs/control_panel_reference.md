...
| **Emit Dashboard Cross-Filters** | Checkbox | `true`         | Emette eventi di filtro nativi di Superset (`setDataMask`) con supporto alla selezione multipla (checkbox/click) e raggruppamento per colonna (`IN`) per filtrare l'intera dashboard in tempo reale. |
